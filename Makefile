...
SELFHOST_SRCS := $(SELFHOST_DIR)/lexer.cb $(SELFHOST_DIR)/parser.cb $(SELFHOST_DIR)/ast.cb $(SELFHOST_DIR)/codegen.cb $(SELFHOST_DIR)/optimizer.cb $(SELFHOST_DIR)/main.cb
...
#   - Assemble with nasm and link with system C linker
bootstrap_nasm: stage0
	...
	./$(GEE) -f nasm $(SELFHOST_DIR)/optimizer.cb $(SELFHOST_DIR)/optimizer.asm
	...
	nasm -felf64 $(SELFHOST_DIR)/optimizer.asm -o $(SELFHOST_DIR)/optimizer.nasm.o
	...
	$(CC) -no-pie -o gee_stage1_nasm $(SELFHOST_DIR)/lexer.nasm.o $(SELFHOST_DIR)/parser.nasm.o $(SELFHOST_DIR)/ast.nasm.o $(SELFHOST_DIR)/codegen.nasm.o $(SELFHOST_DIR)/optimizer.nasm.o $(SELFHOST_DIR)/main.nasm.o stdlib/io.o stdlib/memory.o stdlib/net.o stdlib/system.o
