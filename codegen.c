...
void emit_ir_text(IRProgram *ir, FILE *out) {
    for (size_t i = 0; i < ir->count; ++i) {
        IRInst *in = &ir->insts[i];
        fprintf(out, "%04zu: op=%d a=%d b=%d c=%d text=%s\n", i, (int)in->op, in->a, in->b, in->c, in->text);
    }
}
...
void optimize_ir(IRProgram *ir) {
    /* Stage0 optimization pass:
     *  - remove duplicate labels
     *  - remove jmp to immediately following label
     *  - collapse consecutive NOP records
     */
    if (ir->count < 2) return;
    size_t w = 0;
    for (size_t r = 0; r < ir->count; ++r) {
        IRInst cur = ir->insts[r];
        if (w > 0) {
            IRInst *prev = &ir->insts[w - 1];
            if (prev->op == IR_LABEL && cur.op == IR_LABEL && strcmp(prev->text, cur.text) == 0) {
                continue;
            }
            if (prev->op == IR_NOP && cur.op == IR_NOP) {
                continue;
            }
            if (prev->op == IR_JMP && cur.op == IR_LABEL) {
                char expected[32];
                snprintf(expected, sizeof(expected), "L%d", prev->a);
                if (strcmp(expected, cur.text) == 0) {
                    /* Drop redundant jump and keep current label. */
                    w--;
                }
            }
        }
...
void emit_nasm_x86_64(IRProgram *ir, FILE *out) {
    /* NASM backend path for users who want a strict NAS toolchain. */
    fprintf(out, "default rel\n");
    fprintf(out, "section .text\n");
    /* Emit extern declarations for called symbols so NASM can assemble modules independently. */
    const char *defined[256];
    size_t defined_count = 0;
    for (size_t i = 0; i < ir->count; ++i) {
        IRInst *in = &ir->insts[i];
        if (in->op == IR_LABEL && strncmp(in->text, "fn_", 3) == 0 && defined_count < 256) {
            defined[defined_count++] = in->text + 3;
        }
    }

    const char *externs[256];
    size_t extern_count = 0;
    for (size_t i = 0; i < ir->count; ++i) {
        IRInst *in = &ir->insts[i];
        if (in->op != IR_CALL) continue;
        int is_internal = 0;
        for (size_t d = 0; d < defined_count; ++d) {
            if (strcmp(defined[d], in->text) == 0) { is_internal = 1; break; }
        }
        if (is_internal) continue;
        int seen = 0;
        for (size_t j = 0; j < extern_count; ++j) {
            if (strcmp(externs[j], in->text) == 0) { seen = 1; break; }
        }
        if (!seen && extern_count < 256) externs[extern_count++] = in->text;
    }
    for (size_t i = 0; i < extern_count; ++i) fprintf(out, "extern %s\n", externs[i]);
    ...
    for (size_t i = 0; i < ir->count; ++i) {
        ...
        switch (in->op) {
            ...
            case IR_STORE:
                fprintf(out, "    ; %s\n", in->text);
                fprintf(out, "    ; TODO(nasm): stack-slot store lowering\n");
                break;
            case IR_LOAD:
                fprintf(out, "    ; %s\n", in->text);
                fprintf(out, "    ; TODO(nasm): stack-slot load lowering\n");
                break;
            case IR_RET:
                fprintf(out, "    ret\n");
                break;
