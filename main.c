...
int main(int argc, char **argv) {
    if (argc < 2) {
        fprintf(stderr, "Usage: gee [-f gas|nasm] <input.cb> [output.s|output.asm]\n");
        return 1;
    }
    ...
    if (argc >= 4 && strcmp(argv[1], "-f") == 0) {
        if (strcmp(argv[2], "gas") == 0) backend = BACKEND_GAS;
        else if (strcmp(argv[2], "nasm") == 0) backend = BACKEND_NASM;
        else {
